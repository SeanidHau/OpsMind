...
def test_embedding_client_is_disabled_without_embedding_model(
    isolated_settings: Callable[..., Settings],
) -> None:
    """未配置模型时，应用不应隐式创建外部客户端。"""
    assert create_embedding_client(isolated_settings()) is None
...
def test_embedding_client_requires_a_key_when_model_is_configured(
    isolated_settings: Callable[..., Settings],
) -> None:
    """配置模型但缺少密钥时必须在启动前失败。"""
    with pytest.raises(EmbeddingConfigurationError, match="embedding_api_key"):
        create_embedding_client(isolated_settings(embedding_model="text-embedding-3-small"))
